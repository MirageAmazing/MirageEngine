	...
	namespace Math {
		...
		void Camera::CalculateProjectionMatrix(eCameraProjectionType type){
			if (mProjectionType == eCameraProjectionType::Perspertive && mProjectionType == type)
				mProjectionMatrix = mProjPerspertiveType == eCameraProjPerspertiveType::PerspertiveV1 ? 
					Matrix4x4f::PerspertiveLHv1(mFOV, mAspect, mNear, mFar) : 
					Matrix4x4f::PerspertiveLHv2(mFOV, mAspect, mNear, mFar);
